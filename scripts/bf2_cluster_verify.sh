...
hw_offload_check() {
    ...
    local state
    state=$(ssh "${bf_ssh}" \
        "ovs-vsctl get Open_vSwitch . other_config:hw-offload 2>/dev/null" 2>/dev/null || echo "")
    if [ "${state}" = '"true"' ]; then
        return 0
    fi
    return 1
...
check "tianjin-BF2 hw-offload=true" hw_offload_check tianjin "root@${BF_IP}"
check "fujian-BF2 hw-offload=true"  hw_offload_check fujian \
    "${USER}@${FUJIAN_IP} ssh root@${BF_IP}"
check "helong-BF2 hw-offload=true"  hw_offload_check helong \
    "${USER}@${HELONG_IP} ssh root@${BF_IP}"
...
# Detect RDMA device on each BF2
detect_rdma_dev() {
    local bf_ssh="$1"
    ssh "${bf_ssh}" \
        "for dev in /sys/class/infiniband/mlx5_*; do \
            if [ -e \$dev ]; then basename \$dev; break; fi; done" 2>/dev/null
}
...
run_rdma_lat() {
    ...
    local sdev cdev
    sdev=$(detect_rdma_dev "${server_ssh}")
    cdev=$(detect_rdma_dev "${client_ssh}")

    if [ -z "${sdev}" ] || [ -z "${cdev}" ]; then
        echo "no RDMA device detected"
        return 1
    fi

    # Server in background
    ssh "${server_ssh}" "pkill -f ib_write_lat 2>/dev/null; sleep 1" >/dev/null 2>&1
    ssh -f "${server_ssh}" \
        "nohup ib_write_lat -d ${sdev} -F -s 128 -n 200 >/tmp/ibwl_server.log 2>&1 &"
    sleep 2
    ...
    ssh "${server_ssh}" "pkill -f ib_write_lat 2>/dev/null" >/dev/null 2>&1
...
out=$(run_rdma_lat "root@${BF_IP}" "${USER}@${FUJIAN_IP} ssh root@${BF_IP}" "192.168.56.102")
rc=$?
echo "${out}"
total=$((total+1))
...
out=$(run_rdma_lat "root@${BF_IP}" "${USER}@${HELONG_IP} ssh root@${BF_IP}" "192.168.56.102")
rc=$?
echo "${out}"
total=$((total+1))
if [ $rc -eq 0 ]; then pass_count=$((pass_count+1)); else fail_count=$((fail_count+1)); fi
